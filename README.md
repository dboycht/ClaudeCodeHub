...
### v1.00.3 (2026-08-15)
- ✨ 新增：对话迁移功能 — 将对话迁移到另一个项目目录，Claude Code `/resume` 中可见
  - 自动重命名项目 slug 文件夹
  - 自动改写会话文件内的 `cwd` 字段（大小写不敏感匹配）
  - 同步 `history.jsonl` 的 project 字段和 `sessions/*.json` 元数据
- ✨ 新增：高对比暗色 / 高对比亮色主题（护眼），界面整体放大 15%，照顾视弱用户
- ✅ 确认：删除操作全部走回收站通道
